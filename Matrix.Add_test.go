...
func TestMatrix_Add(t *testing.T) {
	...
	t.Run("Add two matrices", func(t *testing.T) {
		var (
			A   *Matrix
			B   *Matrix
			C   *Matrix
			...
			t.Run("Matrix C", func(t *testing.T) {
				...
				dumpMatrix(C)
				{
					if v, err = C.Get(0, 0); err != nil || v != 0 {
						t.Fatalf("value mismatch (0,0):%v | %v", v, err)
					}
					if v, err = C.Get(0, 1); err != nil || v != 2 {
						t.Fatalf("value mismatch (0,1):%v | %v", v, err)
					}
					if v, err = C.Get(0, 2); err != nil || v != 4 {
						t.Fatalf("value mismatch (0,2):%v | %v", v, err)
					}
					if v, err = C.Get(0, 3); err != nil || v != 6 {
						t.Fatalf("value mismatch (0,3):%v | %v", v, err)
					}
				}
				{
					if v, err = C.Get(1, 0); err != nil || v != 8 {
						t.Fatalf("value mismatch (1,0):%v | %v", v, err)
					}
					if v, err = C.Get(1, 1); err != nil || v != 10 {
						t.Fatalf("value mismatch (1,1):%v | %v", v, err)
					}
					if v, err = C.Get(1, 2); err != nil || v != 12 {
						t.Fatalf("value mismatch (1,2):%v | %v", v, err)
					}
					if v, err = C.Get(1, 3); err != nil || v != 14 {
						t.Fatalf("value mismatch (1,3):%v | %v", v, err)
					}
				}
				{
					if v, err = C.Get(2, 0); err != nil || v != 16 {
						t.Fatalf("value mismatch (2,0):%v | %v", v, err)
					}
					if v, err = C.Get(2, 1); err != nil || v != 18 {
						t.Fatalf("value mismatch (2,1):%v | %v", v, err)
					}
					if v, err = C.Get(2, 2); err != nil || v != 20 {
						t.Fatalf("value mismatch (2,2):%v | %v", v, err)
					}
					if v, err = C.Get(2, 3); err != nil || v != 22 {
						t.Fatalf("value mismatch (2,3):%v | %v", v, err)
					}
				}
				{
					if v, err = C.Get(3, 0); err != nil || v != 24 {
						t.Fatalf("value mismatch (3,0):%v | %v", v, err)
					}
					if v, err = C.Get(3, 1); err != nil || v != 26 {
						t.Fatalf("value mismatch (3,1):%v | %v", v, err)
					}
					if v, err = C.Get(3, 2); err != nil || v != 28 {
						t.Fatalf("value mismatch (3,2):%v | %v", v, err)
					}
					if v, err = C.Get(3, 3); err != nil || v != 30 {
						t.Fatalf("value mismatch (3,3):%v | %v", v, err)
					}
				}
			})
		})
